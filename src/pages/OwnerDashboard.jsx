...
export default function OwnerDashboard() {
  ...
  const approveBooking = async (booking) => {
    ...
    await apiFetch(`/api/bookings/${booking.id}/approve`, {
      token,
      method: 'POST',
    })
  ...
  const activeListings = myVehicles.filter((v) => v.status === 'active' && v.available).length
          ...
          <div className="space-y-6">
            ...
            {myVehicles.length === 0 && (
              <div className="text-center py-16 bg-surface-container-lowest border border-outline-variant rounded-2xl">
                ...
                <Link to="/vendor/add-vehicle" className="bg-primary-container text-white px-6 py-2.5 rounded-xl font-bold hover:opacity-90 transition-all">
                  + List Your Vehicle
                </Link>
              </div>
            )}
                  ...
                  <div key={v.id} className="bg-surface-container-lowest border border-outline-variant rounded-2xl overflow-hidden">
                    ...
                    <div className="p-4">
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${v.status === 'active' && v.available ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                          {v.status === 'active' ? (v.available ? 'Available' : 'Booked') : 'Pending'}
                        </span>
                      </div>
                      ...
                      <div className="flex gap-2 mt-4">
                        ...
                        >
                          {v.status !== 'active' ? 'Awaiting Approval' : v.available ? 'Mark Unavailable' : 'Mark Available'}
                        </button>
                      </div>
                    </div>
                  </div>
